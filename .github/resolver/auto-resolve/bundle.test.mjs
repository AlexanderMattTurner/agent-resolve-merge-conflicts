...
// set, and again with `--verify` appended over every resolution — so the two
// call lines below are this one minus `pnpm`, and asserting them proves the
// caller's command reached the tool rather than a default this tree kept. No
// `-s`: the command belongs to the caller, and a silencing flag added here
// would hide the re-derivation's own output in the run log.
...
// It defaults to the argv, which is what a caller wants to read; `gh` overrides
// it to expand the status-comment body its argv carries only as a file path.
...
function runBundle(
  ...
  const run = spawnSync("python3", [script], {
    ...
    env: {
      ...
      // workflow passes through as `pre-pass-command`. The fake `pnpm` above is
      // what every pre-pass and `--verify` assertion below reads, so the default
...
test("a deferred generated file regenerated by the pre-pass bundles cleanly", () => {
  ...
  writeFileSync(join(work, "a.md"), "resolved\n");
  const { error, merging, bundle, pnpmCalls } = runBundle(work, "a.md", {
  ...
  // The caller's own command, run verbatim and then again with `--verify`.
...
// ---------------------------------------------------------------------------
// A caller that declared NO pre-pass command. This resolver runs for other
// repositories, and one with no generated files has nothing to re-derive — so
...
test("a deferred generated file with no pre-pass command is REFUSED, not bundled", () => {
  ...
  // caller that HAS derived files and named no command that re-derives them.
  // Bundling would ship whatever the model typed into a build output.
  const { work } = midMerge({
    base: { "a.md": "base\n", "gen.txt": "generated base\n" },
    feature: { "a.md": "feature side\n", "gen.txt": "generated feature\n" },
    main: { "a.md": "main side\n", "gen.txt": "generated main\n" },
  ...
  writeFileSync(join(work, "a.md"), "resolved\n");
  const { error, bundle, ghCalls, pnpmCalls } = runBundle(work, "a.md", {
    env: { DEFERRED_REGEN: "gen.txt", AUTO_RESOLVE_PRE_PASS: "" },
  });
  assert.notEqual(error, null);
  assert.equal(existsSync(bundle), false);
...
test("a caller with no pre-pass command and nothing deferred bundles, running no generator", () => {
  // The other direction, which keeps the refusal above from hardening into "a
  // repository without generators cannot call this resolver". There is no
  // generator to compare against, so there is no content post-condition either.
  const { work } = midMerge();
  writeFileSync(join(work, "a.md"), "resolved: feature + main\n");
  const { error, bundle, pnpmCalls } = runBundle(work, "a.md", {
    env: { AUTO_RESOLVE_PRE_PASS: "" },
  });
  assert.equal(error, null);
  assert.ok(existsSync(bundle));
  assert.deepEqual(pnpmCalls, []);
