...
``_discover_refusals`` words every refusal and writes the ``refused_*`` pair for one PR. ``_discover_gh`` makes every ``gh`` call, so nothing here runs a subprocess.
...
class Notifier:
    ...
    def notify_once(self, number: int, marker: str, body: str) -> None:
        """Post this notice, or REWRITE the one already carrying its marker.

        The rewrite is what stops a notice outliving its own truth. A refusal's
        wording states what the author must do, and a resolver change can move
        that; the marker then holds the stale sentence on the PR forever, and a
        second comment cannot correct the text above it. An unchanged body
        matches and costs no call, so the common scan still writes nothing.
        """
        wanted = f"{body}\n\n{marker}"
        try:
            # The body is BASE64 here because this channel is line-oriented and a
            # notice body carries newlines: read raw, one comment would arrive as
            # several rows and never compare equal, so every scan would rewrite it.
            ...
            return
        endpoint = f"repos/{self.config.repo}/issues/{number}/comments"
        method = "POST"
        for line in existing.splitlines():
            comment_id, _, encoded = line.partition(" ")
            # A row this cannot decode is a protocol fault, not a missing notice:
            # let it raise rather than silently posting a duplicate comment.
            if b64decode(encoded, validate=True).decode("utf-8") == wanted:
                return
            endpoint = f"repos/{self.config.repo}/issues/comments/{comment_id}"
            method = "PATCH"
            break
        try:
            self.gh.run_gh(
