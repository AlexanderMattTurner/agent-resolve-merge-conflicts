...
const DIFF = [
  "diff --git a/dist/bundle.mjs b/dist/bundle.mjs",
...
const OMIT = ["dist/bundle.mjs", "types/"]; // "types/" must hide nothing
...
test("drops a generated file's section and keeps the hand-written one", () => {
  ...
  assert.ok(kept.includes("diff --git a/src/parse.mjs b/src/parse.mjs"));
  assert.ok(!kept.includes("dist/bundle.mjs"));
  ...
  assert.ok(kept.includes("+const parse = 2;"));
});
...
test("a rename with only one generated side is kept", () => {
  const diff = [
    "diff --git a/src/parse.mjs b/dist/bundle.mjs",
