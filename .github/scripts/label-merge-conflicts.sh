...
#
# Env: GH_TOKEN, REPO; PR_NUMBER scopes to one PR; BASE_REF scopes a sweep to one
# branch's PRs; MAX_PASSES (default 2) caps the retry loop; RETRY_DELAY_SECS the
# between-pass wait; SWEEP_LIMIT (default 100) the full-repo sweep listing.
set -euo pipefail
...
fetch_page() {
  ...
  retry_stdout gh pr list --repo "$REPO" --state open --limit "$SWEEP_LIMIT" \
    ${scope[@]+"${scope[@]}"} --json number,mergeable,labels,headRefOid,baseRefName
}
