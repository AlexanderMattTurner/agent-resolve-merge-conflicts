...
# Decide-gate resolution. A crashed or cancelled decide job leaves `run` empty,
# which the skip branch below would misread as "no relevant changes" and report
# GREEN with nothing verified. Only a clean 'success' or legitimate 'skipped'
# proceeds; a cancelled decide is benign only under supersession (checked
# first); anything else, including an unwired `needs.<decide>.result`, is RED.
