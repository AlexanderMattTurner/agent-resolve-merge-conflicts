...
3. **If none exists, prefer a check that iterates the SSOT and asserts the second copy**, so it covers every future member for free and never names the instance — the same SSOT preference [`code-style.md`](../../rules/code-style.md) applies to existing duplication. A generic guard is an SSOT-completeness check, not a per-case assertion.
