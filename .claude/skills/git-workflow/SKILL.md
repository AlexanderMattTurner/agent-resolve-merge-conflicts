...
**After any merge of the base onto the head — the bot's or your own — read the remaining PR diff as a NEW proposal: every hunk must still be necessary for this PR's goal under the base's CURRENT design.** The delta audit above judges what the merge changed; this judges what the branch still needs. Delete a hunk the base has superseded, and adapt the callers to the replacement. A modify/delete conflict is where this costs most: keeping a file the base deleted restores a retired mechanism, so carry the feature's behaviour on the replacement instead, and say in the PR body when you keep the file anyway.
...
**Do not poll for completion at all — the task-completion notification is the only signal you need, and it always arrives.** Banned in every dress: a `sleep` before the read, a loop over `git status`, a re-read "just to check", and `pgrep`.
