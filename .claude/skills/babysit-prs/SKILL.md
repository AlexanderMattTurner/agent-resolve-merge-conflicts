...
**You never merge** — root [`CLAUDE.md`](../../../CLAUDE.md)'s "Never merge a PR unless the user directly and explicitly tells you to" binds here without exception, whatever the PR's state, and it covers `merge_pull_request` and `gh pr merge` alike. This holds even when the user's stated goal _arithmetically requires_ merges ("get open PRs under 3"): deliver everything short of the merge, then say what remains and ask. An exclusion is not a grant — "don't merge the big one" authorizes nothing about the others.
...
**The queue clears `auto_merge` and blanks mergeability.** A PR reading `unknown` plus `auto_merge: null` plus all checks green is typically mid-flight, and both "enable auto-merge" and "merge" will be refused with a merge-queue-specific error. Treat that error on an otherwise-green PR as _confirmation the PR is fine_, not as a failure to route around.
...
The automated reviewer casts no vote: it posts a COMMENT review whose findings are inline threads, and the required `Review findings resolved` status is red while any thread carrying a gating severity (🔴 blocking, 🟡 warning) is unresolved. That signal is exactly the one a session drops: it arrives from `github-actions[bot]` amid the repo's other bot comments, it looks like the chatter the auto-unsubscribe rule says to skip, and it commonly lands after the session already unsubscribed or ended.

- **An unresolved review thread on your PR is excluded from "bot chatter."** It never counts toward the auto-unsubscribe counter, and a live gating thread forbids auto-unsubscribing. A bot author is not a reason to skip a review.
- **PULL the state; don't wait to be pushed it.** On any turn touching a PR you own — at minimum before every "CI is green" / "ready to merge" / "done" claim, and on the first turn after any resubscribe — read the reviews and review threads explicitly (`pull_request_read` methods `get_reviews` and `get_review_comments`), not just check runs. `mergeable_state: blocked` does **not** distinguish a queued check from a gate waiting on a thread, so a session that reads only checks blames CI and waits forever on a PR that is waiting on _it_.
- **Addressing a finding means pushing the fix AND replying on the thread, then resolving it.** Resolving is what greens the gate. Never "fix" a finding by dismissing the review: the gate reads threads, so a dismissal clears nothing.
- **Resolving a thread fires no workflow event, so the status stays stale until something re-evaluates it.** REMOVE the `review-gate-recheck` label and add it again — `labeled` fires only on a transition, so a bare re-add over a label still sitting there fires nothing. Use the REST route (`gh api -X DELETE repos/{owner}/{repo}/issues/<N>/labels/review-gate-recheck`, then `gh api -X POST repos/{owner}/{repo}/issues/<N>/labels -f 'labels[]=review-gate-recheck'`); a 404 on the DELETE means the label was not there.
