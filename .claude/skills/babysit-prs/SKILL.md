...
**You never merge** — root `CLAUDE.md`'s "never merge without explicit instruction" rule binds here without exception, whatever the PR's state. This holds even when the user's stated goal _arithmetically requires_ merges ("get open PRs under 3"): deliver everything short of the merge, then say what remains and ask. An exclusion is not a grant — "don't merge the big one" authorizes nothing about the others.
