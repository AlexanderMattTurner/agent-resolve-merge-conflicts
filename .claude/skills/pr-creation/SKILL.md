...
**Combine work onto one branch while that branch stays under ~50 changed files — count them with `git diff --name-only "$CLAUDE_CODE_BASE_REF"...HEAD | wc -l`. Past that bound, split.** Combining is worth doing because every PR boots the full workflow fan-out on a shared runner pool, so two small PRs cost about twice the CI of one, and each one's required checks queue behind the other's long jobs. That saving stops paying as the branch grows: a large branch touches files the trunk is also touching, so it conflicts before it lands, and each re-resolution spends the whole fan-out again. Land related and merely co-discovered work together while the bound holds. Split when the branch would pass ~50 files, when a piece must land on its own timeline, or when the diff is too large to review coherently. A change that cannot be cut into independently valid pieces — a rename and its call sites — stays whole and exceeds the bound; splitting it would open PRs that do not build on their own. One big PR still owes the reviewer legibility: one commit per separable concern, plus a `## Partitions` map in the body.

**The bound is a price, not a taste — so it has a retirement condition.** It trades CI spend against conflict rate, and both sides move independently, so the two directions have different triggers. It was set against a trunk taking ~166 merges a day at a median of 13 changed files per merge, so ~50 is about four times the median. **Raise the file count, or drop the bound,** when trunk velocity falls well under ~100 merges a day, or when a merge driver removes the generated-file conflicts — both make a large branch cheaper to land. **Lower it** when the per-PR fixed cost drops (most workflows job-gated so an untouched area costs seconds), which makes splitting cheap.
...
**Skip the `## Lessons Learned` section entirely when THIS repo is the template — and read the repo name, never the prose.** Run `git remote get-url origin` and compare the name it prints with the repo that `TEMPLATE_REPO` names in `.github/workflows/phone-home.yaml`, which a fork redirects with `vars.TEMPLATE_SYNC_ORG`. A repo made from the template often still carries the template's own opening line in `CLAUDE.md`, so that text is not evidence of where you are. In the template a lesson propagates nothing, because the change is already there. Everywhere else phone-home is the only channel, and it runs one way into the template, so omitting the section drops the lesson for good.
...
Resolving a thread fires no workflow event at all, so the required `Review findings resolved` status keeps its old verdict until something re-evaluates it. REMOVE the `review-gate-recheck` label and add it again — `labeled` is the trigger `review-findings-gate.yaml` listens for, and it fires only on a transition, so a bare re-add over a label still sitting on the PR does nothing. Use the REST route: `gh api -X DELETE repos/{owner}/{repo}/issues/<N>/labels/review-gate-recheck`, then `gh api -X POST repos/{owner}/{repo}/issues/<N>/labels -f 'labels[]=review-gate-recheck'`. A 404 on the DELETE means the label was not there. The reviewer reads each PR ONCE, so no push buys a second review; `needs-auto-review` buys a fresh read when you want one.
