...
Resolving a thread fires no event `review-gate.yaml` listens for, so the `Automated review posted` status stays stale until your next push re-runs it. The reviewer's own hold clears on the twice-hourly sweep (`claude-reviewer-hold-clear.yaml`) once no reviewer thread is unresolved. The reviewer reads each PR ONCE, so no push buys a second review. A hold whose concern lived only in the review BODY opens no thread for the sweep to rest on: clear it by adding the `needs-auto-review` label, which buys a fresh read, or by dismissing the review, which does the same.
