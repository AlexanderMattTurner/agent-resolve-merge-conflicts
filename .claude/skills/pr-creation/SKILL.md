...
**Skip the `## Lessons Learned` section entirely when the repo this PR MERGES INTO is the template.** phone-home runs in that repo, so the base repo decides and your checkout does not — from a fork, read the PR's base repo; otherwise `git remote get-url origin` answers it. Compare that name with the repo `TEMPLATE_REPO` names in `.github/workflows/phone-home.yaml`, which `vars.TEMPLATE_SYNC_ORG` redirects for a fork-hosted template. Never take a repo's own `CLAUDE.md` prose as evidence: a repo made from the template often still carries the template's opening line. In the template a lesson propagates nothing, because the change is already there. Everywhere else phone-home is the only channel, and it runs one way into the template, so omitting the section drops the lesson for good.
