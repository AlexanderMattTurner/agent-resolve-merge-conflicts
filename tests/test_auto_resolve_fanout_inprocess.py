...
# --- the fan-out's width is what the runner holds, not a constant ------------


def _meminfo(tmp_path: Path, available_kb: int) -> Path:
    """A `/proc/meminfo` carrying MemAvailable, in the real file's shape: the
    wanted row is neither first nor alone, so a reader that takes line one or
    matches `Mem` loosely is caught."""
    path = tmp_path / "meminfo"
    path.write_text(
        "MemTotal:       16376932 kB\n"
        "MemFree:          123456 kB\n"
...
def test_a_runner_too_small_for_one_shard_still_runs_one(tmp_path, monkeypatch):
...
def test_an_unreadable_meter_leaves_the_width_alone(tmp_path, monkeypatch):
    """No meter is not no memory. A runner whose procfs carries no MemAvailable
