...
if [[ "$(uname -s) $(uname -m)" = "Linux x86_64" ]]; then
  ...
  # PATH is prepended for the installer alone: its post-condition is that bare
  # `mergiraf` resolve into the destination, which auto-resolve's pre-pass needs
  # and this checkout does not — the driver names an absolute path. `timeout`
  # because curl's --connect-timeout does not cap an established transfer, so a
  # stalled download would otherwise hang the whole setup.
  if ! PATH="${mergiraf_dest}:${PATH}" timeout --kill-after=10 300 \
    bash .github/scripts/install-mergiraf.sh "$mergiraf_dest"; then
...
if [[ "$(git config core.hooksPath)" = ".hooks" ]]; then
  ...
  echo ""
  echo "Next steps:"
  echo "  1. Rewrite the first line of CLAUDE.md to name THIS repo, then add your project details"
  if [[ -f package.json ]]; then
